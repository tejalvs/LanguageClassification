...
dutch_articles = ['een', 'de', 'het', 'groene', 'groen', 'hij', 'zij', 'haar', 'hem', 'zijn', 'dit', 'deze', 'die',
                  'dat', 'wie', 'wat']
...
common_dutch = ['niet', 'wat', 'ze', 'zijn', 'maar', 'die', 'heb', 'voor', 'ben', 'mijn', 'dit', 'hem', 'hebben',
                'heeft', 'nu',
...
def get_feature9(i, curr_data):
    ...
    for single_word in ind_words:
        single_word = single_word.lower().replace(',', '')
        if (single_word.endswith('r') or single_word.endswith('i') or single_word.endswith('n') or single_word.endswith(
                'e')):
            return False
    return True
